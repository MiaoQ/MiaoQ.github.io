...
module.exports = {
  ...
  plugins: [
    new CleanWebpackPlugin(), // 生成打包文件之前先清空dist文件夹
    new HTMLPlugin({title: 'output management'}), // 自动生成index.html
    new VueLoaderPlugin()
  ]
}
```
...
9. 由于添加了[html-webpack-plugin](https://github.com/jaketrent/html-webpack-template)插件，在dist目录下自动生成了index.html  
```html
<!DOCTYPE html>
<html>
  <head>
...
10. 在webpack.dev.js中书写配置，让代码在本地跑起来
```jsx
const path = require('path')
const VueLoaderPlugin = require('vue-loader/lib/plugin')
...
11. 在package.json中添加运行本地服务的脚本命令
```jsx
"scripts": {
